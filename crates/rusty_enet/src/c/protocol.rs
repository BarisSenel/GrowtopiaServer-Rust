...
unsafe fn enet_protocol_handle_connect<S: Socket>(
    ...
    {
        crate::debug_println!("DEBUG: Connect rejected - Invalid channel count: {}", channel_count);
    ...
    while current_peer < ((*host).peers).add((*host).peer_count) {
        if (*current_peer).state == ENET_PEER_STATE_DISCONNECTED as i32 as u32 {
            ...
            if (*current_peer)
                ...
                .assume_init_ref()
                .as_ref()
                .is_some_and(|a| a.same_host((*host).received_address.assume_init_ref().as_ref().unwrap()))
                && (*current_peer).connect_id == (*command).connect.connect_id
            {
                // Duplicate connection from same address/port/id - ENet standard behavior
                return current_peer;
            }
    ...
    if peer.is_null() || duplicate_peers >= (*host).duplicate_peers {
        crate::debug_println!("DEBUG: Connect rejected - Peer null or duplicate limit: null={}, dupe={}", peer.is_null(), duplicate_peers);
        return core::ptr::null_mut();
    }
...
unsafe fn enet_protocol_handle_incoming_commands<S: Socket>(
    ...
    crate::debug_println!("TRACE: header_size={}, peer_id={}, flags={:04X}, len={}", header_size, peer_id, flags, (*host).received_data_length);
    ...
    if peer_id as i32 == PROTOCOL_MAXIMUM_PEER_ID as i32 {
        ...
        let same_addr = (*host).received_address.assume_init_ref().as_ref().unwrap().same((*peer).address.assume_init_ref().as_ref().unwrap());
        if (*peer).state == ENET_PEER_STATE_DISCONNECTED as i32 as u32
            || (*peer).state == ENET_PEER_STATE_ZOMBIE as i32 as u32
            || !(*host)
            .received_address
    ...
    if flags as i32 & ENET_PROTOCOL_HEADER_FLAG_COMPRESSED as i32 != 0 {
        ...
        crate::debug_println!("TRACE: Decompressed size={}", original_size);
        if original_size <= 0_i32 as usize
            || original_size > ::core::mem::size_of::<[u8; 4096]>().wrapping_sub(header_size)
        {
            crate::debug_println!("TRACE: Decompression failed or too large");
            return false;
        }
        if (*host).using_new_packet_server {
            copy_nonoverlapping(
    ...
    if let Some(checksum_fn) = (*host).checksum.assume_init_ref() {
        ...
        let calculated = checksum_fn(&in_buffers);
        if calculated != desired_checksum {
            crate::debug_println!("TRACE: Checksum MISMATCH! (carrying on for debug)");
            // return false; 
        }
    }
    if !peer.is_null() {
    ...
    while current_data < ((*host).received_data).add((*host).received_data_length) {
        ...
        crate::debug_println!("TRACE: Received command={}, seq={}", command_number, u16::from_be((*command).header.reliable_sequence_number));
        ...
        match command_number as i32 {
            ...
            2 => {
                if !peer.is_null() {
                    crate::debug_println!("TRACE: Command CONNECT but peer is NOT null!");
                    break;
                }
                crate::debug_println!("TRACE: Calling enet_protocol_handle_connect...");
                peer = enet_protocol_handle_connect(host, header, command);
                if peer.is_null() {
                    crate::debug_println!("TRACE: enet_protocol_handle_connect returned NULL!");
                    break;
                }
                crate::debug_println!("TRACE: enet_protocol_handle_connect returned PEER {}", (*peer).incoming_peer_id);
            }
            3 => {
...
unsafe fn enet_protocol_send_outgoing_commands<S: Socket>(
    ...
    if (*host).using_new_packet {
        let host_port = Address::port(&(*host).socket.assume_init_ref().address());
        
        let rand1: u16 = (enet_host_random(host) as u16) % (host_port + 1);

        unsafe {
            (*new_header).integrity[0] = rand1.to_be();
            (*new_header).integrity[1] = (rand1 ^ host_port).to_be();
